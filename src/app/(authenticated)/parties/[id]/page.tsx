...
export default function VendorDetailPage() {
  ...
  const fetchContacts = async () => {
    try {
      const res = await fetch(`/api/vendors/${params.id}/contacts`)
      if (res.ok) {
        const data = await res.json()
  ...
  const runLEXA = async () => {
    ...
    try {
      ...
      if (res.ok) {
        await fetchVendor()
        alert('Party profile created successfully!')
      }
    } catch (error) {
  ...
  if (!vendor) {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-bold">Party not found</h2>
        <Link href="/parties">
          <Button className="mt-4">Back to Parties</Button>
        </Link>
      </div>
    )
  }
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="font-medium font-mono">{vendor.dunsNumber || '—'}</p>
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <p className="font-medium">{vendor.stateProvince || '—'}</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{vendor.businessOwner || '—'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Associate/Paralegal</p>
                <p className="font-medium">{vendor.itOwner || '—'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-3">
              ...
              {vendor.primaryContactEmail && (
                <div>
                  ...
                  <p className="font-medium">{vendor.primaryContactEmail}</p>
                </div>
              )}
              {vendor.primaryContactPhone && (
                <div>
                  <p className="text-sm text-gray-500">Phone</p>
                  <p className="font-medium">{vendor.primaryContactPhone}</p>
                </div>
              )}
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Open Issues ({vendor.riskFindings.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
